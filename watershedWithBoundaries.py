...
class Image:
    ...
    def ifGetNeighborPixelsFourShouldMark(self, p, CEarr):
        ...
        if p0i >= 0 and p0i < len(self.image) and p0j >= 0 and p0j < len(self.image[0]):
            a[0] = [p0i,p0j]
            if CEarr[p0i][p0j][0] != CEarr[p[0]][p[1]][0] and CEarr[p0i][p0j][1] != CEarr[p[0]][p[1]][1]:
                flag = True

        if p1i >= 0 and p1i < len(self.image) and p1j >= 0 and p1j < len(self.image[0]):
            a[1] = [p1i,p1j]
            if CEarr[p1i][p1j][0] != CEarr[p[0]][p[1]][0] and CEarr[p1i][p1j][1] != CEarr[p[0]][p[1]][1]:
                flag = True

        if p2i >= 0 and p2i < len(self.image) and p2j >= 0 and p2j < len(self.image[0]):
            a[2] = [p2i,p2j]
            if CEarr[p2i][p2j][0] != CEarr[p[0]][p[1]][0] and CEarr[p2i][p2j][1] != CEarr[p[0]][p[1]][1]:
                flag = True

        if p3i >= 0 and p3i < len(self.image) and p3j >= 0 and p3j < len(self.image[0]):
            a[3] = [p3i,p3j]
            if CEarr[p3i][p3j][0] != CEarr[p[0]][p[1]][0] and CEarr[p3i][p3j][1] != CEarr[p[0]][p[1]][1]:
                flag = True
    ...
    def watershedMR1998(self, draw):
        ...
        for i in range(len(picture.image)):
            for j in range(len(picture.image[0])):
                if self.ifGetNeighborPixelsFourShouldMark([i, j], CEarr):
                    draw.image[i][j] = [ 255, 255, 255 ] 
             
        # for i in range(len(picture.image)):
        #     for j in range(len(picture.image[0])):
        #         if resolved_picture.image[i][j][0] == 0:
        #             resolved_picture.image[i][j] = [ 255, 255, 255 ]
        #         else:
        #             resolved_picture.image[i][j] = draw.image[i][j]
...
def main ():
    ...
    #img.read_image('smoothImage.jpg')
    # img.read_image('topological.png')
    # img.read_image('check.png')

    pictureToDrawOnBoundaries = Image()
    pictureToDrawOnBoundaries.read_image('grayscale.jpg')

    pictureToDrawOnBoundaries.wrap_boundary_copy()
    pictureToDrawOnBoundaries.wrap_boundary_copy()
    ...
    def onclick(event):
        if event.xdata != None and event.ydata != None:
            ...
            plt.figure(2)
            plt.title('Lower Complete Image')
            plt.imshow(picture.image)

            picture2 = Image()
            picture2 = picture.watershedMR1998(pictureToDrawOnBoundaries)
            pictureToDrawOnBoundaries.write_image('watershedWithBoundaries')

            plt.figure(3)
            plt.title('Watershed')
            plt.imshow(pictureToDrawOnBoundaries.image)
            plt.show()
