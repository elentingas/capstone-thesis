...
class Image:
    ...
    def regionGrowing(self, x, y, fg, picture):
        ...
        while not pixels.empty():
            ...
            pixel = (np.int32)(self.image[current[0], current[1]][0])
...
def main ():
    ...
    def onclick(event):
        ...
        if event.xdata != None and event.ydata != None:
            ...
            img.regionGrowing(xx, yy, fg, picture)

            plt.figure(2)
            plt.imshow(picture.image)
            plt.title('Region Growing')
            plt.show()
            picture.write_image('regionGrowingMultipleClicks')
